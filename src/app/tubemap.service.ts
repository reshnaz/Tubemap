...
import tubemapconfig from '../tubemapconfig.json';
...
export class TubemapService {
  ...
  // tube = {
    ...
    adjustHeight(height, stationCount, options) {
    ...
    adjustWidth(width, stationCount, options){
    ...
    drawDots(svg, dots, options?: any) {
      svg.selectAll("dot")
        .data(dots)
        .enter().append("circle")
        .attr("r", (d)=>{ return d.pointRadius; })
        .attr("cx", (d)=> { return d.x; })
        .attr("cy", (d)=> { return d.y; })
        .attr("class", (d)=> { return d.className || ''; })
        .style("fill", (d)=>{ return d.pointColor; })
        .style("stroke", (d)=>{ return d.borderColor; })
        .style("stroke-width", (d)=>{ return d.borderWidth; });
    }
  
    drawRects(svg, rects, options?: any){
      _.each(rects, (r)=>{
        svg.append("rect")
          .attr("width", r.width)
    ...
    drawLabels(svg, labels, options) {
      svg.selectAll("text")
        ...
        .attr("alignment-baseline",(d)=>{ return d.alignment; })
        .attr("dominant-baseline",(d)=>{ return d.alignment; })
        .attr("font-size", (d)=>{ return d.fontSize; })
        .style("font-family", (d)=>{ return d.fontFamily; })
        .style("font-weight", (d)=>{ return d.fontWeight; })
        .style("fill", (d)=>{ return d.textColor; });
    }
  
    drawLines(svg, lines, options) {
    ...
    drawMap(lines, legend, width, height, options){
      ...
      dots = _.filter(points, (p)=>{ return p.pointRadius && p.pointRadius > 0; });
      if (showLabels) labels = _.filter(points, (p)=>{ return p.label !== undefined || p.symbol !== undefined; });
      rects = _.filter(points, (p)=>{ return p.hubSize; });
    ...
    getLengths(xDiff, yDiff, directions, y, options) {
      var lengths = [],
          rand = this.helper.hRandom(20, 80) / 100,
          yUnit = options.yUnit,
          paddingY = options.padding[1],
          i = 0, timeout = 10,
          firstY;
    ...
    getNextX(boundaries, iterator, totalPoints, width, minXDiff, prevPoint?){
      var x = 0,
          prevPadding = 0.25,
          trendPadding = 0.4,
          tempPercent = iterator/totalPoints,
          percentComplete = parseFloat(tempPercent.toString()),
      ...
      do {
        ...
        if (minX<maxX) {
          x =  this.helper.hRandom(minX, maxX);
        } else {
          x =  this.helper.hRandom(maxX, minX);
        }
        if (prevPoint)
    ...
    getPointsBetween(p1, p2, pathTypes, cornerRadius, options) {
      var that = this,
          points = [],
          x1 = p1.x, y1 = p1.y,
          x2 = p2.x, y2 = p2.y,
      ...
      pathTypes = _.filter(pathTypes, (pt)=>{
      ...
      if (pathType && xDirection) {
        ...
        // retrieve directions
        var directions = tubemapconfig.pathTypes;
        ...
        _.each(directions, (direction, i)=>{
          var length = lengths[i],
              point: Point = that.translateCoordinates(x, y, direction, length),
              pointR1 = {}, pointR2 = {};
    ...
    getTitleLines(title, titleMaxLineChars) {
      var lines = [],
    ...
    makeLines(stations, width, height, options){
      var that = this,
          // options
          paddingX = options.padding[0],
          paddingY = options.padding[1],
      ...
      _.each(stations, (station, i)=>{
        ...
        _.each(station.lines, (lineLabel, j)=>{
    ...
    processStations(stations){
      ...
      _.each(stations, (station, i)=>{
        ...
        station.lines = _.sortBy(station.lines, (lineLabel)=>{ return lineLabels.indexOf(lineLabel); });
      });
